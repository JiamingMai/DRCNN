...
clear; close all;
%% settings
folder = 'examples/DRCNN_train/';
model = [folder 'DRCNN_mat.prototxt'];
weights = [folder 'models/v2/DRCNN_iter_1091500.caffemodel'];
...
for idx = 1 : layers
    ...
    if channel == 1
        weights = ones(fsize^2, fnum);
    else
        weights = ones(channel, fsize^2, fnum);
    end
